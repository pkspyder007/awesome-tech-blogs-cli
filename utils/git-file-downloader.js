...
class GitFileDownloader {
	...
	constructor(options) {
		const optSchema = joi
			.object()
			.keys({
				provider: joi.string().allow(['github', 'gitlab']).required(),
				repository: joi.string().required(),
				branch: joi.string().default('master'),
				file: joi.string().required(),
				outputName: joi.string().default(null),
		...
		if (validatedOptions.error) {
			throw new Error(
				this.constructor.name +
					...
					validatedOptions.error.message
			);
		}
		...
		switch (this.options.provider) {
			...
			default:
				this.url =
					GitFileDownloader.GithubRawBaseUrl +
					[
				...
				if (this.options.oauth2_token !== null) {
					this.headers.Authorization =
						'token ' + this.options.oauth2_token;
				}
				break;

			case 'gitlab':
				this.url =
					GitFileDownloader.GitlabRawBaseUrl +
					[
						this.options.repository,
						'raw',
						this.options.branch,
	...
	run() {
		return new Promise((resolve, reject) => {
			const req = request.get(this.url).redirects(0);
			...
			// Setup Basic Auth
			if (
				this.options.basic_username !== null &&
				this.options.basic_password !== null
			) {
				req.auth(
					this.options.basic_username,
					this.options.basic_password
				);
			}
			...
			req.end((requestError, res) => {
				if (!requestError) {
					if (this.options.output === null) {
						...
						let fileSubPath = path.basename(
							this.options.outputName ?? this.options.file
						);
					...
					requestError.message =
						'Error while downloading file. Please check options and token.';
					reject(requestError);
				}
			});
		});
...
export default GitFileDownloader;
